...
class CustomUserManager(BaseUserManager):
    def create_user(self, email, first_name, last_name, password=None, profile_photo=None, **extra_fields):
        if not email:
            raise ValueError('The Email field must be set')
        email = self.normalize_email(email)
        user = self.model(email=email, first_name=first_name, last_name=last_name, profile_photo=profile_photo, **extra_fields)
...
class User(AbstractBaseUser):
    ...
    profile_photo = models.CharField(max_length=255, null=True, blank=True)  # Store avatar URL as a string
